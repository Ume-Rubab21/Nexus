...
import "./received-reqs.css";

type FriendRequest = {
  _id: string;
  sender: { name: string; email: string; department?: string };
  status: string;
};

type ReceivedReqsProps = {
  onLogout?: () => void;
};

const ReceivedReqs: React.FC<ReceivedReqsProps> = ({ onLogout }) => {
    ...
    const fetchReceivedRequests = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `http://localhost:5000/api/friendrequests/received?email=${encodeURIComponent(currentEmail)}`
        );

        const data = await res.json();
        if (!res.ok) throw new Error(data.message || "Failed to fetch received requests");
  ...
  try {
    const res = await fetch(`http://localhost:5000/api/friendrequests/respond`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id, idType: "_id", action }),
    });
  ...
  if (loading)
    return (
      <>
        ...
        <p className="loading-text">Loading received requests...</p>
      </>
    );
      ...
      <div className="received-requests-container">
        ...
        {requests.map(req => (
          <div className="request-card" key={req._id}>
            <p>
              <strong>Name:</strong>{" "}
              <span className="clickable" onClick={() => goToProfile(req.sender.email)}>
                {req.sender.name}
              </span>
            </p>
            <p><strong>Email:</strong> {req.sender.email}</p>
            <p><strong>Department:</strong> {req.sender.department || "—"}</p>
            <p><strong>Status:</strong> {req.status}</p>
            <div className="button-group">
              <button className="btn-accept" onClick={() => respondToRequest(req._id, "Accept")}>Accept</button>
              <button className="btn-reject" onClick={() => respondToRequest(req._id, "Reject")}>Reject</button>
            </div>
          </div>
        ))}
        </div>
